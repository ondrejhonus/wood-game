...
\begin{document}
    ...
    \noindent Tato závěrečná práce dokumentuje proces vývoje videohry s~názvem Wood Game v~herním enginu Unity. 
    Text se zaměřuje především na návrh herní logiky, ale také na tvorbu 2D grafických prvků (např. uživatelského rozhraní) 
    v~programu Aseprite, kompozici hudby v~Bitwig Studiu a~nahrávání zvukových efektů v~Audacity. 
    Proces tvorby 3D modelů zde není popsán, jelikož nebyl realizován mnou.

    Cílem projektu bylo vytvořit funkční hru zaměřenou na těžbu dřeva, ekonomický systém a~fyziku prostředí. 
    Uživatel ovládá postavu, která interaguje s~herním světem a~používá zakoupené nástroje a~vybavení.
    ...
    Wood Game in the Unity environment. The development process primarily covers backend elements, but also the creation of 2D graphics, music, and sound effects, excluding the 3D graphical part which was not created by me.
    ...
    \vspace*{-6em} % Adjust this value to reduce top padding (e.g., -1em, -1.5em, etc.)
    \tableofcontents
    \vspace*{-10em} % Adjust this value to reduce top padding (e.g., -1em, -1.5em, etc.)
    ...
    \thispagestyle{fancy}
    Cílem práce bylo vytvořit videohru, ve které může uživatel ve 3D prostředí kácet stromy, které se vyskytují na různých místech a~mají různou hodnotu.
    Dřevo lze následně prodat za herní měnu, za kterou si může koupit lepší nástroje pro rychlejší kácení stromů, nebo také automobil pro přepravu dřeva.
    Uživatel má také možnost uložit svůj postup a~načíst jej později.

    Videohra měla původně obsahovat funkci hry pro více hráčů, ale z~časových důvodů jsem se rozhodl tuto funkci vypustit.

    Téma tvorby videohry jsem si zvolil, protože mi tvorba videoher nebyla příliš vzdálená a~dostal jsem nabídku spolupráce od spolužáka, 
    který se na vývoji hry podílel po grafické stránce.
    ...
    Těchto indikátorů může být více, pokud hráč začne klikat na jiné místo, než na které počátečně kliknul.
    ...
    if (Physics.Raycast(ray, out RaycastHit hit, 100f, chopLayer)) {
        ...
        if (hit.collider.gameObject == gameObject && Vector3.Distance(playerInventory.transform.position, hit.point) <= chopRange) {
            // Spustit funkci na přeseknutí stromu
    ...
    Hráč může kdykoliv během hry uložit svůj postup pomocí tlačítka "Save Game" v~menu.
    ...
    Všechny objekty (pokácené stromy) na jeho pozemku jsou poté obnoveny do stejné pozice, ve které je hráč nechal.
    ...
    Pokladna funguje tak, že když hráč přinese objekt s~označením "Sellable" (prodejné) na pokladnu, tak se mu strhne částka z~jeho peněz.
    Každý prodejný objekt má připojený skript, který obsahuje cenu daného objektu.
    Po prodeji objektu zmizí označení "Sellable", aby hráč nemohl objekt koupit znovu.
    ...
    Menu je navrženo tak, aby bylo přehledné a~intuitivní, s~jednoduchou navigací.
    ...
    Cílem této závěrečné práce bylo vytvořit funkční 3D videohru v~prostředí Unity, která bude obsahovat těžbu dřeva, 
    ekonomický systém a~možnost ukládání postupu. Tento cíl se podařilo naplnit. 
    Výsledkem je hratelná videohra Wood Game.

    Během vývoje jsem narazil na řadu výzev, zejména v~oblasti interakce objektů a~optimalizace herních skriptů. 
    Řešení těchto problémů mi umožnilo výrazně zlepšit mé znalosti v~oblasti objektově orientovaného programování, 
    řešení algoritmů a~návrhu herní struktury. Také jsem si vyzkoušel proces tvorby hry od úplného začátku.

    Vzhledem k~časové náročnosti projektu považuji současnou verzi hry za pouhý základ, 
    který se stále potřebuje vyvíjet. Jsem si vědom určitých nedostatků, zejména v~oblasti fyziky nošení předmětů, 
    která je v~současné verzi problematická kvůli kolizím. Také vzhled 
    stromů by do budoucna vyžadoval lepší zpracování, například přidání větví, jelikož současné stromy jsou pouhé klády s~listím.

    Do budoucna plánuji projekt dále rozvíjet. Kromě zmíněných oprav chyb je v~plánu přidání režimu pro více hráčů (multiplayer), 
    který byl původně zvažován, a~rozšíření herního světa o~nové lokace a~nástroje.

    I~přes zmíněné možnosti pro budoucí vylepšení považuji projekt za úspěšný. 
    Všechny klíčové mechaniky fungují a~hra poskytuje uspokojivý uživatelský zážitek.
