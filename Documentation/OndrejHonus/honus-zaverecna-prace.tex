...
\begin{document}
    ...
    Data se ukládají do formátu JSON. 2D grafika a uživatelskéhé rozhraní jsou vytvořeny v~programu Aseprite.
    ...
    Téma tvorby videohry jsem si zvolil, protože mi je svět herního vývoje blízký a~dostal jsem nabídku spolupráce od spolužáka, který se na vývoji hry podílel po grafické
    stránce. Také jsem ale chtěl rozšířit  své znalosti v~oblasti objektového programování a~práce s~herním enginem. 
    Byla to pro mě příležitost vyzkoušet si komplexní proces vývoje od návrhu architektury až po finální ladění.

    Na projektu jsem spolupracoval se spolužákem. Měli jasně rozdělené role. 
    ...
    \begin{figure}[h]
        ...
        \caption{Ukázka regálu se sekerama a~cenovkou.}
        \label{fig:obchod}
    \end{figure}
    ...
    Soundtrack je dostupný na YouTube pod názvem "Wood Game OST | Fair Sapling" na adrese: \url{https://www.youtube.com/watch?v=x9g16xBjPlY}.
    ...
    od interakce s~objektama, kácení stromů, přes správu inventáře a~ekonomiky, až po ukládání dat.
    
    Ačkoliv se jedná o~prototyp, hra obsahuje všechny důležité prvky a~je plně hratelná. 
